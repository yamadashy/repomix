...
describe('calculateOutputMetrics', () => {
  ...
  it('should process large content in parallel', async () => {
    ...
    const expectedChunks = getProcessConcurrency() * 2;
    expect(chunksProcessed).toBe(expectedChunks); // CPU-proportional chunk count
    expect(result).toBe(expectedChunks * 100); // chunks * 100 tokens per chunk
  });
  ...
  it('should correctly split content into chunks for parallel processing', async () => {
    ...
    expect(processedChunks.length).toBe(expectedChunks); // CPU-proportional chunk count
