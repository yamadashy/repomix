...
describe('calculateOutputMetrics', () => {
  ...
  it('should process large content in parallel', async () => {
    ...
    const expectedNumChunks = Math.max(2, getProcessConcurrency() * 2);
    expect(chunksProcessed).toBe(expectedNumChunks); // Should match CPU cores * 2
    expect(result).toBe(expectedNumChunks * 100); // numChunks * 100 tokens per chunk
  });
  ...
  it('should correctly split content into chunks for parallel processing', async () => {
    ...
    // Number of chunks should match CPU cores * 2
    const expectedNumChunks = Math.max(2, getProcessConcurrency() * 2);
    const chunkSizes = processedChunks.map((chunk) => chunk.length);

    expect(processedChunks.length).toBe(expectedNumChunks);
