...
describe.runIf(!isWindows)('CLI Line Limit Integration Tests', () => {
  ...
  test('should parse --line option correctly', async () => {
    ...
    await expect(
      runCli([tempDir], tempDir, { line: 10 })
  ...
  test('should handle line limit with different output formats', async () => {
    ...
    await expect(
      runCli([tempDir], tempDir, { line: 10, style: 'xml' })
    ).resolves.not.toThrow();

    // Test with Markdown output
    await expect(
      runCli([tempDir], tempDir, { line: 10, style: 'markdown' })
    ).resolves.not.toThrow();

    // Test with Plain output
    await expect(
      runCli([tempDir], tempDir, { line: 10, style: 'plain' })
    ).resolves.not.toThrow();

    // Test with JSON output
    await expect(
      runCli([tempDir], tempDir, { line: 10, style: 'json' })
    ).resolves.not.toThrow();
  });
  ...
  test('should handle line limit with other CLI options', async () => {
    ...
    await expect(
      runCli([tempDir], tempDir, { line: 10, verbose: true })
    ).resolves.not.toThrow();

    // Test with copy to clipboard
    await expect(
      runCli([tempDir], tempDir, { line: 10, copy: true })
    ).resolves.not.toThrow();

    // Test with remove comments
    await expect(
      runCli([tempDir], tempDir, { line: 10, removeComments: true })
    ).resolves.not.toThrow();

    // Test with remove empty lines
    await expect(
      runCli([tempDir], tempDir, { line: 10, removeEmptyLines: true })
    ).resolves.not.toThrow();

    // Test with compress
    await expect(
      runCli([tempDir], tempDir, { line: 10, compress: true })
    ).resolves.not.toThrow();
  });
  ...
  test('should handle line limit with file selection options', async () => {
    ...
    await expect(
      runCli([tempDir], tempDir, { line: 10, include: '*.js' })
    ).resolves.not.toThrow();

    // Test with ignore pattern
    await expect(
      runCli([tempDir], tempDir, { line: 10, ignore: '*.md' })
    ).resolves.not.toThrow();

    // Test with no gitignore
    await expect(
      runCli([tempDir], tempDir, { line: 10, gitignore: false })
    ).resolves.not.toThrow();

    // Test with no default patterns
    await expect(
      runCli([tempDir], tempDir, { line: 10, defaultPatterns: false })
    ).resolves.not.toThrow();
  });
  ...
  test('should handle line limit with output options', async () => {
    ...
    await expect(
      runCli([tempDir], tempDir, { line: 10, output: outputFile })
    ).resolves.not.toThrow();

    // Test with stdout
    await expect(
      runCli([tempDir], tempDir, { line: 10, stdout: true })
    ).resolves.not.toThrow();

    // Test with show line numbers
    await expect(
      runCli([tempDir], tempDir, { line: 10, outputShowLineNumbers: true })
    ).resolves.not.toThrow();

    // Test with parsable style
    await expect(
      runCli([tempDir], tempDir, { line: 10, parsableStyle: true })
    ).resolves.not.toThrow();
  });
  ...
  test('should handle line limit with security options', async () => {
    ...
    await expect(
      runCli([tempDir], tempDir, { line: 10, securityCheck: false })
    ).resolves.not.toThrow();
  });
  ...
  test('should handle line limit with git options', async () => {
    ...
    await expect(
      runCli([tempDir], tempDir, { line: 10, gitSortByChanges: false })
    ).resolves.not.toThrow();

    // Test with include diffs
    await expect(
      runCli([tempDir], tempDir, { line: 10, includeDiffs: true })
    ).resolves.not.toThrow();

    // Test with include logs
    await expect(
      runCli([tempDir], tempDir, { line: 10, includeLogs: true })
    ).resolves.not.toThrow();

    // Test with include logs count
    await expect(
      runCli([tempDir], tempDir, { line: 10, includeLogsCount: 25 })
    ).resolves.not.toThrow();
  });
  ...
  test('should handle line limit with token count options', async () => {
    ...
    await expect(
      runCli([tempDir], tempDir, { line: 10, tokenCountEncoding: 'cl100k_base' })
    ).resolves.not.toThrow();

    // Test with token count tree
    await expect(
      runCli([tempDir], tempDir, { line: 10, tokenCountTree: true })
    ).resolves.not.toThrow();

    // Test with token count tree threshold
    await expect(
      runCli([tempDir], tempDir, { line: 10, tokenCountTree: 100 })
    ).resolves.not.toThrow();
  });
  ...
  test('should handle line limit with top files length option', async () => {
    ...
    await expect(
      runCli([tempDir], tempDir, { line: 10, topFilesLen: 20 })
    ).resolves.not.toThrow();
  });
  ...
  test('should handle line limit with header text option', async () => {
    ...
    await expect(
      runCli([tempDir], tempDir, { line: 10, headerText: 'Custom Header Text' })
    ).resolves.not.toThrow();
  });
  ...
  test('should handle line limit with instruction file path option', async () => {
    ...
    await expect(
      runCli([tempDir], tempDir, { line: 10, instructionFilePath: instructionFile })
    ).resolves.not.toThrow();
  });
  ...
  test('should handle line limit with directory structure options', async () => {
    ...
    await expect(
      runCli([tempDir], tempDir, { line: 10, includeEmptyDirectories: true })
    ).resolves.not.toThrow();

    // Test with include full directory structure
    await expect(
      runCli([tempDir], tempDir, { line: 10, includeFullDirectoryStructure: true })
    ).resolves.not.toThrow();
  });
  ...
  test('should handle line limit with truncate base64 option', async () => {
    ...
    await expect(
      runCli([tempDir], tempDir, { line: 10, truncateBase64: true })
    ).resolves.not.toThrow();
  });
  ...
  test('should handle line limit with multiple directories', async () => {
    ...
    await expect(
      runCli([dir1, dir2], tempDir, { line: 10 })
    ).resolves.not.toThrow();
  });
  ...
  test('should handle line limit with stdin option', async () => {
    ...
    await expect(
      runCli([tempDir], tempDir, { line: 10, stdin: true })
    ).resolves.not.toThrow();
  });
});