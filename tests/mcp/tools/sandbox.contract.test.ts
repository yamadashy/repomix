...
// Look-alikes that are VALID filenames and must NOT be over-rejected: a leading
...
describe('sandbox contract', () => {
  ...
  describe('confinement: pack directory + include/ignore patterns', () => {
    ...
    test('pack applies the untrusted-input guards to the CLI (config isolation + search backstop)', async () => {
      // Behavioral proxy for the untrusted-config + confinement contract. The true
      // blackbox check (a workspace config's instructionFilePath NOT reaching the
      // output) needs the real pack pipeline and lives in the e2e suite; here we
      // assert the tool never lets the workspace/global config load and always
      // confines the file search to the root.
      await pack({ directory: '.', compress: false, topFilesLength: 10, style: 'xml' });
      expect(vi.mocked(runCli).mock.calls[0][2]).toMatchObject({
        skipLocalConfig: true,
        skipGlobalConfig: true,
        confineToBaseDir: true,
        // Disable git-based change sorting so the pack never spawns `git log` inside
        // the untrusted workspace (honoring its .git/config is a subprocess surface).
