...
describe("defaultAction", () => {
  beforeEach(() => {
    ...
    vi.mocked(processConcurrency.initTaskRunner).mockReturnValue(
      mockTaskRunner,
    );
  });
  ...
  it("should run the default command successfully", async () => {
    ...
    const taskRunner = vi.mocked(processConcurrency.initTaskRunner).mock
      .results[0].value;
  ...
  it("should handle errors gracefully", async () => {
    ...
    await expect(
      runDefaultAction(["."], process.cwd(), options),
  ...
  describe("buildCliConfig", () => {
    ...
    it("should properly trim whitespace from comma-separated patterns", () => {
      ...
      expect(config.ignore?.customPatterns).toEqual([
        "node_modules/**",
        "dist/**",
        "coverage/**",
      ]);
