  ...
  for (const file of files) {
    ...
    while (i < content.length) {
      if (content.charCodeAt(i) === 96) {
        let count = 1;
        while (i + count < content.length && content.charCodeAt(i + count) === 96) count++;
        if (count > maxBackticks) maxBackticks = count;
        i += count;
      } else {
        i++;
      }
...
export const calculateFileLineCounts = (processedFiles: ProcessedFile[]): Record<string, number> => {
