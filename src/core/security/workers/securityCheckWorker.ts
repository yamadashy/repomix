...
// The fast pre-filter (`mightContainSecret` and its underlying patterns) lives
// in `../securityCheckPrefilter.ts` so it can be shared with the main thread
// (the security-result cache pre-partitions files there) without importing
// `@secretlint/core`. Re-exported here for the existing behavior-preservation
// test that cross-checks the pre-filter against the live engine.
export { mightContainSecret } from '../securityCheckPrefilter.js';
