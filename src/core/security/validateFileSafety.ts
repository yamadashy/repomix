...
export const validateFileSafety = async (
  ...
  externalTaskRunner?: TaskRunner<SecurityCheckTask, SuspiciousFileResult | null>,
  deps = {
    runSecurityCheck,
  ...
  if (config.security.enableSecurityCheck) {
    ...
    const allResults = await deps.runSecurityCheck(
      ...
      gitDiffResult,
      gitLogResult,
      externalTaskRunner ? { initTaskRunner: undefined as never, taskRunner: externalTaskRunner } : undefined,
    );
