...
import { createMetricsTaskRunner, type MetricsTaskRunnerWithWarmup } from '../../core/metrics/calculateMetrics.js';
...
export const runDefaultAction = async (
  ...
  // Speculatively warm up metrics workers with the default encoding while
  // migration check and config loading run sequentially below. Workers need
  // ~300ms for BPE initialization; starting here gains ~20ms of overlap
  // with migration + config I/O that would otherwise be dead time.
  const speculativeEncoding = (cliOptions.tokenCountEncoding ?? 'o200k_base') as TokenEncoding;
