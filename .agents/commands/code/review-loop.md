...
1. **Review** — Spawn 5 agents in parallel, each reviewing the current diff from a different angle:
   ...
   - **Agent 4 — Conventions**: Project conventions (CLAUDE.md), naming, structure, test coverage
...
4. **Verify** with `npm run lint` and `npm run test`. Fix any regressions before continuing.
