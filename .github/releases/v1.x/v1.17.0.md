...
- Language hints are now resolved for extensionless filenames in subdirectories (for example `docs/Makefile`), instead of falling back to no hint.
- Code fences are widened so a git diff containing a triple-backtick block can no longer break out of its fence.
