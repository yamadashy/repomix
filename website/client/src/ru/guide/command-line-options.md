...
| `--remote-trust-config` | Доверять и загружать файлы конфигурации из удалённых репозиториев. Доверенная конфигурация может выполнять команды и читать локальные файлы, поэтому используйте её только для репозиториев, которым полностью доверяете (по умолчанию отключено из соображений безопасности) |
