...
- `--sandbox [dir]`: (вместе с `--mcp`) Ограничить файловые инструменты MCP-сервера рабочей директорией (по умолчанию — текущая рабочая директория; например, `--sandbox path/to/project`). Все пути указываются относительно этого корня, абсолютные пути и пути хоста отклоняются, а упаковка удалённых репозиториев, генерация Skills и присоединение внешних выходных файлов отключены. См. [MCP-сервер](/ru/guide/mcp-server)
