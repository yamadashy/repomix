...
- **Каждый путь указывается относительно корня рабочей директории.** Абсолютные пути, `~`, `..` и пути Windows с буквой диска или UNC-пути отклоняются, а пути, которые разрешаются за пределы корня (в том числе через символические ссылки), отбрасываются. Результаты и сообщения об ошибках также относительные, поэтому пути хоста не раскрываются.
