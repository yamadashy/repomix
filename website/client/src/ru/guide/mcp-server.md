...
По умолчанию MCP-сервер может читать любой путь, доступный пользователю хоста. Это удобно для доверенного локального ассистента, но слишком широко, если сервер доступен недоверенному клиенту или агенту. Флаг `--sandbox` ограничивает файловые инструменты сервера единственной рабочей директорией:

```bash
# Ограничить текущей рабочей директорией
repomix --mcp --sandbox

# Ограничить конкретной директорией
repomix --mcp --sandbox path/to/project
```

Когда режим песочницы включён:

- **Каждый путь указывается относительно корня рабочей директории.** Абсолютные пути, `~`, `..` и пути Windows с буквой диска или UNC-пути отклоняются, а пути, которые разрешаются за пределы корня (в том числе через символические ссылки), отбрасываются. Результаты и сообщения об ошибках также относительные, поэтому пути хоста не раскрываются. Это касается аргументов `directory` и `path` в справочнике по инструментам ниже: в режиме песочницы указывайте их относительно корня рабочей директории, а не в виде абсолютных путей, как это обычно описано в этих таблицах.
- **Регистрируются только доступные для чтения инструменты, ограниченные корневой директорией:** `pack_codebase`, `read_repomix_output`, `grep_repomix_output`, `file_system_read_file` и `file_system_read_directory`. Упаковка удалённых репозиториев, генерация Skills и присоединение внешних выходных файлов отключены, поскольку они обращаются к сети, записывают файлы или ссылаются на произвольные пути. Сами инструменты `file_system_*` доступны только в режиме песочницы, где корень рабочей директории ограничивает то, к чему они могут получить доступ.

Это ограничение поверхности инструментов на уровне приложения (эшелонированная защита), а не изоляция на уровне ОС. При хостинге сервера для недоверенных клиентов всё равно используйте обычную изоляцию вашей платформы (контейнеры, выделенные пользователи).

`--sandbox` влияет только на MCP-сервер и не действует без `--mcp`.
...
- Обеспечивает доступ к упакованной кодовой базе без необходимости доступа к файловой системе
...
Эти два инструмента файловой системы доступны только в [режиме песочницы](#режим-песочницы) (`--sandbox`), где корень рабочей директории ограничивает то, к чему они могут получить доступ. Без `--sandbox` они не регистрируются.

1. `file_system_read_file`
  - Читает содержимое файла по пути относительно корня рабочей директории (например, `src/index.ts`)
  - Отклоняет содержимое, соответствующее известным форматам секретов ([Secretlint](https://github.com/secretlint/secretlint)), в качестве дополнительной эвристической меры защиты; граница доступа — это корень рабочей директории, а не сканирование
  - Возвращает понятные сообщения об ошибках для недопустимых путей, не раскрывая пути хоста

2. `file_system_read_directory`
  - Выводит содержимое директории по пути относительно корня рабочей директории (например, `.` или `src`)
...
- Анализировать конкретные файлы в рабочей директории
