...
В примере выше (`compress: true` как глобальное значение по умолчанию) файлы в `src/core/` сохраняют полное содержимое, файлы в `docs/` показаны только в дереве директорий, а всё остальное сжимается.
