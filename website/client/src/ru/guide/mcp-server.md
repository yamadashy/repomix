...
Эти два инструмента файловой системы доступны только в [режиме песочницы](#режим-песочницы) (`--sandbox`), где корень рабочей директории ограничивает то, к чему они могут получить доступ. Без `--sandbox` они не регистрируются, поэтому сервер по умолчанию не предоставляет никакого прямого чтения файлов.
