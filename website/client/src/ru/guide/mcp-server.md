...
Важно понимать, что MCP-сервер защищает, а что нет, потому что именно ИИ-агент, а не вы, решает, какой инструмент вызвать и с какими аргументами.

- **Инструменты читают всё, что может прочитать процесс.** `pack_codebase` упаковывает любой переданный ему абсолютный путь к директории, а упакованный вывод возвращается агенту. Нет ограничения корнем проекта; границей служат права доступа к файлам вашей операционной системы.
- **Сканирование секретов — это эвристика, а не контроль доступа.** [Secretlint](https://github.com/secretlint/secretlint) исключает файлы, соответствующие известным форматам учётных данных (например, ключи AWS и приватные ключи). Он распознаёт не все секреты, поэтому такие файлы, как `~/.netrc` или kubeconfig, могут пройти через него незамеченными. Считайте чистую проверку признаком того, что «ничего очевидного не найдено», а не того, что «это безопасно передавать».
- **Содержимое, которое читает агент, может влиять на его поведение.** Если он анализирует ненадёжный репозиторий, текст в этом репозитории может попытаться указать ему упаковать конфиденциальную директорию или клонировать не связанный с задачей URL. Repomix отказывается клонировать эндпоинты метаданных облачных инстансов, но не может отличить настоящий запрос от внедрённого.

Подключайте MCP-сервер только к тем агентам и репозиториям, которым вы готовы предоставить такой уровень доступа.
...
4. **Продвинутые функции**: Использует все возможности Repomix, такие как сжатие кода, подсчёт токенов и сканирование секретов.
