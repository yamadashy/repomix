...
- Они включены только при локальном запуске CLI (запуск `repomix` на вашей собственной машине), где граница доверия такая же, как у npm-скриптов или Makefile.
- Они **отключены** для библиотечного API (`pack()` / `runCli()`), MCP-сервера и размещённого сервиса [repomix.com](https://repomix.com).
- Для удалённых репозиториев (`--remote`) конфигурация клонированного репозитория считается доверенной только при одновременном использовании `--remote-trust-config`.

Активные обработчики логируются при запуске, чтобы неожиданные обработчики из незнакомой конфигурации были заметны.
:::

Примечания:

- Не рекомендуется сочетать обработчик с `output.compress` (или `compress` в `output.patterns`) для одного и того же файла: преобразованное содержимое может больше не парситься как исходный язык. Сжатие выполняется по принципу best-effort и незаметно откатывается к преобразованному содержимому при ошибке парсинга.
