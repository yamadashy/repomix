...
| `output.splitOutput`             | Разделить вывод на несколько нумерованных файлов по максимальному размеру на часть. При использовании CLI укажите читаемые размеры, такие как `500kb` или `2mb`. Это сохраняет каждый файл в пределах лимита и предотвращает разделение файлов между частями | Не задано |
