...
| `input.processors`               | `{ pattern, command, timeout?, onError? }`エントリの順序付き配列で、パッキング前に外部コマンドを実行してマッチしたファイルを変換します（例：JSON→TOON）。最初にマッチしたglobが優先されます。任意のコマンドを実行するため、ローカルCLI実行時のみ有効です。[ファイルプロセッサー](#ファイルプロセッサー)を参照してください | 未設定                 |
...
- ローカルCLI実行時（自分のマシンで`repomix`を実行する場合）にのみ有効になり、この信頼境界はnpmスクリプトやMakefileと同じです。
- ライブラリAPI（`pack()` / `runCli()`）、MCPサーバー、およびホストされている[repomix.com](https://repomix.com)では**無効**です。
- リモートリポジトリ（`--remote`）の場合、クローンされたリポジトリの設定は、`--remote-trust-config`も渡された場合にのみ信頼されます。

有効なプロセッサーは起動時にログに記録されるため、見慣れない設定からの予期しないプロセッサーが可視化されます。
:::

注意事項：

- 同じファイルに対してプロセッサーと`output.compress`（または`output.patterns`の`compress`）を組み合わせることは推奨されません。変換後の内容は、元の言語として解析できなくなる可能性があります。圧縮はベストエフォートであり、解析に失敗した場合は変換後の内容に静かにフォールバックします。
