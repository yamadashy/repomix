...
- **AI最適化**: コードベースを構造化されたセクションと明確な構成でAIが理解しやすい形式にフォーマット化
- **トークンカウント**: OpenAIのtiktokenなどの設定可能なトークナイザーを使用してLLMのコンテキスト制限に対応するためのトークン数を計測
- **Git対応**: `.gitignore`および`.git/info/exclude`ファイルを自動的に認識して不要なファイルをパッケージング対象から除外
- **セキュリティ重視**: Secretlintを使用した機密情報の検出と保護により、意図しない情報漏洩を防止
- **複数の出力形式**: XML（AI向け最適）、Markdown（読みやすさと構造のバランス）、プレーンテキストから選択可能
- **コード圧縮**: 実装の詳細を省きながら本質的なコード構造を抽出し、トークン数を削減
