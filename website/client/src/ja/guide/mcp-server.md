...
- ファイルシステムアクセスを必要とせずにパッケージ化されたコードベースへのアクセスを提供
- 大きなファイルの部分読み込みをサポート
...
MCPサーバーが何を保護し、何を保護しないのかを理解しておくことが重要です。どのツールをどの引数で呼び出すかを決めるのは、あなたではなくAIエージェントだからです。

- **ツールは、プロセスが読み取れるものをそのまま読み取ります。** `pack_codebase`は指定された任意の絶対パスのディレクトリをパッケージ化し、パッケージ化された出力はエージェントに返されます。プロジェクトルート内への閉じ込めはなく、境界となるのはOSのファイル権限だけです。
- **シークレットスキャンはヒューリスティックであり、アクセス制御ではありません。** [Secretlint](https://github.com/secretlint/secretlint)は、既知の認証情報の形式（AWSキーや秘密鍵など）に一致するファイルを除外します。すべてのシークレットを認識できるわけではないため、`~/.netrc`やkubeconfigのようなファイルはすり抜けることがあります。スキャンで問題が見つからなかった場合も「明らかな問題は見つからなかった」という意味であり、「共有しても安全」という意味ではありません。
- **エージェントは読み取った内容に誘導される可能性があります。** 信頼できないリポジトリを分析すると、そのリポジトリ内のテキストが、機密ディレクトリをパッケージ化させたり、無関係なURLをクローンさせたりするようエージェントに指示しようとすることがあります。Repomixはクラウドインスタンスのメタデータエンドポイントへのクローンを拒否しますが、正当なリクエストと注入されたリクエストを区別することはできません。

MCPサーバーは、この程度のアクセスを許可してもよいと判断できるエージェントとリポジトリに対してのみ接続してください。
...
4. **高度な機能**: コード圧縮、トークンカウント、シークレットスキャンなど、Repomixのすべての機能を活用できます。
