...
| `input.processors`               | 依序排列的 `{ pattern, command, timeout?, onError? }` 項目陣列，會在打包前執行外部命令來轉換符合的檔案內容（例如 JSON→TOON）。第一個符合的 glob 優先。因為會執行任意命令，所以僅在本地 CLI 執行時啟用。請參閱[檔案處理器](#檔案處理器) | 未設定                 |
...
- 僅在本地 CLI 執行時啟用（在您自己的機器上執行 `repomix`），此時的信任邊界與 npm 腳本或 Makefile 相同。
- 在函式庫 API（`pack()` / `runCli()`）、MCP 伺服器，以及託管的 [repomix.com](https://repomix.com) 中則為**停用**狀態。
- 對於遠端儲存庫（`--remote`），只有在同時傳入 `--remote-trust-config` 時，才會信任所複製儲存庫的設定。

啟動時會記錄目前啟用的處理器，讓來自不熟悉設定檔的意外處理器變得可見。
:::

注意事項：

- 不建議在同一個檔案上同時使用處理器與 `output.compress`（或 `output.patterns` 的 `compress`）：轉換後的內容可能已無法再被解析為原始語言。壓縮是盡力而為，解析失敗時會靜默回退為轉換後的內容。
