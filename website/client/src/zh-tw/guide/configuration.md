...
2. `.repomixignore`
3. `.ignore`（如果`ignore.useDotIgnore`為true且未使用`--no-dot-ignore`）
4. `.gitignore`和`.git/info/exclude`（如果`ignore.useGitignore`為true且未使用`--no-gitignore`）
5. 預設模式（如果`ignore.useDefaultPatterns`為true且未使用`--no-default-patterns`）
