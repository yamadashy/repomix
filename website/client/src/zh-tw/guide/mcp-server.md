...
了解 MCP 伺服器保護什麼、不保護什麼很重要，因為決定呼叫哪個工具、傳入什麼參數的是 AI 代理，而不是您。

- **工具會讀取行程能夠讀取的一切內容。** `pack_codebase` 會打包傳入的任意絕對路徑目錄，打包後的輸出會回傳給代理。它不會被限制在專案根目錄內，唯一的邊界是您作業系統的檔案權限。
- **機密掃描是一種啟發式方法，而非存取控制。** [Secretlint](https://github.com/secretlint/secretlint) 會排除符合已知憑證格式（例如 AWS 金鑰與私密金鑰）的檔案。它無法識別所有機密資訊，因此像 `~/.netrc` 或 kubeconfig 這類檔案可能會漏過偵測。掃描結果乾淨只代表「沒有發現明顯問題」，不代表「可以安全地分享」。
- **代理可能被它讀取的內容操縱。** 如果它分析一個不受信任的儲存庫，該儲存庫中的文字可能會試圖指示它打包某個敏感目錄，或克隆一個無關的 URL。Repomix 會拒絕克隆雲端執行個體中繼資料端點，但它無法分辨請求是真實的還是被注入的。

請只將 MCP 伺服器連接到您願意授予這種存取層級的代理與儲存庫。
...
4. **進階功能**：利用 Repomix 的所有功能，如程式碼壓縮、令牌計數和機密掃描。
