...
理解 MCP 服务器保护什么、不保护什么很重要，因为决定调用哪个工具、传入什么参数的是 AI 代理，而不是你。

- **工具会读取进程能够读取的一切内容。** `pack_codebase` 会打包传入的任意绝对路径目录，打包后的输出会返回给代理。它不会被限制在项目根目录内，唯一的边界是你操作系统的文件权限。
- **机密扫描是一种启发式方法，而不是访问控制。** [Secretlint](https://github.com/secretlint/secretlint) 会排除匹配已知凭据格式（例如 AWS 密钥和私钥）的文件。它无法识别所有机密信息，因此像 `~/.netrc` 或 kubeconfig 这样的文件可能会漏过检测。扫描结果干净只代表“没有发现明显问题”，不代表“可以安全地共享”。
- **代理可能被它读取的内容操纵。** 如果它分析一个不受信任的仓库，该仓库中的文本可能会试图指示它打包某个敏感目录，或克隆一个无关的 URL。Repomix 会拒绝克隆云实例元数据端点，但它无法分辨请求是真实的还是被注入的。

请只将 MCP 服务器连接到你愿意授予这种访问级别的代理和仓库。
...
4. **高级功能**：利用 Repomix 的所有功能，如代码压缩、token 计数和机密扫描。
