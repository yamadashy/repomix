...
**Diff格式标志**（互斥 - 选择一个）：
- `--git-stat`：显示每个文件的更改计数（git log --stat）
- `--git-patch`：显示逐行diff（git log --patch）
- `--git-numstat`：显示每个文件的数字增减（git log --numstat）
- `--git-shortstat`：显示更改的单行摘要（git log --shortstat）
- `--git-dirstat`：显示目录更改分布（git log --dirstat）
- `--git-name-only`：仅显示文件名（git log --name-only）- 默认
- `--git-name-status`：显示带A/M/D/R状态的文件名（git log --name-status）
- `--git-raw`：显示带SHA哈希和模式的底层格式（git log --raw）

**输出详细程度和图表选项**（可与任何diff格式组合）：
- `--git-graph`：包含ASCII和Mermaid提交图可视化（git log --graph --all）
- `--git-summary`：显示文件操作如创建、重命名、模式更改（git log --summary）
