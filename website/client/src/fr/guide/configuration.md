...
| `output.splitOutput`             | Diviser la sortie en plusieurs fichiers numérotés par taille maximale par partie. Lors de l'utilisation du CLI, spécifiez des tailles lisibles comme `500kb` ou `2mb`. Cela maintient chaque fichier sous la limite et évite de diviser les fichiers entre les parties | Non défini |
