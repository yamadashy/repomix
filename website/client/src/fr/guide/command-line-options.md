...
- `--stat` : Afficher le nombre de modifications par fichier (git log --stat)
- `--patch` : Afficher les diffs ligne par ligne (git log --patch)
- `--numstat` : Afficher les ajouts/suppressions numériques par fichier (git log --numstat)
- `--shortstat` : Afficher un résumé d'une ligne des modifications (git log --shortstat)
- `--dirstat` : Afficher la distribution des modifications par répertoire (git log --dirstat)
- `--name-only` : Afficher uniquement les noms de fichiers (git log --name-only) - par défaut
- `--name-status` : Afficher les noms de fichiers avec le statut A/M/D/R (git log --name-status)
- `--raw` : Afficher le format bas niveau avec les hashes SHA et les modes (git log --raw)

**Options de Niveau de Détail et de Graphe** (combinables avec n'importe quel format de diff) :
- `--graph` : Inclure la visualisation du graphe de commits ASCII et Mermaid (git log --graph --all)
- `--summary` : Afficher les opérations sur les fichiers comme les créations, renommages, changements de mode (git log --summary)
