...
डिफ़ॉल्ट रूप से MCP सर्वर वह हर पथ पढ़ सकता है जिसे होस्ट उपयोगकर्ता पढ़ सकता है। यह एक भरोसेमंद स्थानीय असिस्टेंट के लिए सुविधाजनक है, लेकिन जब सर्वर किसी अविश्वसनीय क्लाइंट या एजेंट के सामने उजागर किया जाता है तो यह बहुत व्यापक हो जाता है। `--sandbox` फ्लैग सर्वर के फ़ाइल टूल्स को एक ही वर्कस्पेस डायरेक्टरी तक सीमित कर देता है:

```bash
# वर्तमान वर्किंग डायरेक्टरी तक सीमित करें
repomix --mcp --sandbox

# किसी विशिष्ट डायरेक्टरी तक सीमित करें
repomix --mcp --sandbox path/to/project
```

जब सैंडबॉक्स मोड चालू होता है:

- **हर पथ वर्कस्पेस रूट के सापेक्ष होता है।** पूर्ण (absolute) पथ, `~`, `..`, और Windows ड्राइव/UNC पथों को अस्वीकार कर दिया जाता है, और जो पथ रूट के बाहर हल होते हैं (सिमलिंक के ज़रिए भी) उन्हें हटा दिया जाता है। परिणाम और त्रुटि संदेश भी सापेक्ष ही होते हैं, ताकि होस्ट के पथ उजागर न हों। यह नीचे दिए गए टूल संदर्भ में `directory` और `path` आर्गुमेंट्स पर भी लागू होता है: सैंडबॉक्स मोड में इन्हें वर्कस्पेस रूट के सापेक्ष पास करें, न कि उन पूर्ण पथों के रूप में जो अन्यथा उन तालिकाओं में वर्णित हैं।
- **केवल रीड-ओनली, रूट-सीमित टूल्स ही रजिस्टर किए जाते हैं:** `pack_codebase`, `read_repomix_output`, `grep_repomix_output`, `file_system_read_file`, और `file_system_read_directory`। रिमोट पैकिंग, स्किल जनरेशन, और बाहरी आउटपुट संलग्न करना अक्षम कर दिया जाता है, क्योंकि ये नेटवर्क तक पहुंचते हैं, फ़ाइलें लिखते हैं, या मनमाने पथों का संदर्भ देते हैं। दोनों `file_system_*` टूल्स खुद भी केवल सैंडबॉक्स मोड में ही उपलब्ध होते हैं, जहां वर्कस्पेस रूट यह सीमित करता है कि वे कहाँ तक पहुंच सकते हैं।

यह टूल सतह का एप्लिकेशन-स्तरीय सीमांकन है (defense in depth), OS-स्तरीय सैंडबॉक्स नहीं। जब अविश्वसनीय क्लाइंट्स के लिए सर्वर होस्ट किया जाए, तो भी इसे अपने प्लेटफ़ॉर्म के सामान्य आइसोलेशन (कंटेनर, समर्पित उपयोगकर्ता) के अंतर्गत ही चलाएं।

`--sandbox` केवल MCP सर्वर को प्रभावित करता है; `--mcp` के बिना इसका कोई प्रभाव नहीं होता।
...
- फ़ाइल सिस्टम पहुंच की आवश्यकता के बिना पैकेज्ड कोडबेस तक पहुंच प्रदान करता है
...
ये दोनों फ़ाइल सिस्टम टूल्स केवल [सैंडबॉक्स मोड](#सैंडबॉक्स-मोड) (`--sandbox`) में ही उपलब्ध होते हैं, जहां वर्कस्पेस रूट यह सीमित करता है कि वे कहाँ तक पहुंच सकते हैं। `--sandbox` के बिना ये रजिस्टर नहीं होते।

1. `file_system_read_file`
  - वर्कस्पेस रूट के सापेक्ष किसी पथ पर फ़ाइल सामग्री पढ़ता है (जैसे `src/index.ts`)
  - अतिरिक्त ह्यूरिस्टिक सुरक्षा उपाय के रूप में ज्ञात सीक्रेट फॉर्मेट्स ([Secretlint](https://github.com/secretlint/secretlint)) से मेल खाने वाली सामग्री को अस्वीकार करता है; पहुंच की सीमा वर्कस्पेस रूट है, स्कैन नहीं
  - अमान्य पथों के लिए स्पष्ट त्रुटि संदेश वापस करता है, बिना होस्ट पथ उजागर किए

2. `file_system_read_directory`
  - वर्कस्पेस रूट के सापेक्ष किसी पथ पर डायरेक्टरी की सामग्री सूचीबद्ध करता है (जैसे `.` या `src`)
...
- वर्कस्पेस में विशिष्ट फ़ाइलों का विश्लेषण करना
