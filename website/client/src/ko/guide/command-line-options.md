...
- `--git-stat`: 파일별 변경 횟수 표시 (git log --stat)
- `--git-patch`: 줄별 diff 표시 (git log --patch)
- `--git-numstat`: 파일별 숫자 추가/삭제 표시 (git log --numstat)
- `--git-shortstat`: 변경사항의 한 줄 요약 표시 (git log --shortstat)
- `--git-dirstat`: 디렉토리 변경 분포 표시 (git log --dirstat)
- `--git-name-only`: 파일명만 표시 (git log --name-only) - 기본값
- `--git-name-status`: A/M/D/R 상태와 함께 파일명 표시 (git log --name-status)
- `--git-raw`: SHA 해시와 모드가 포함된 저수준 형식 표시 (git log --raw)

**출력 상세 수준 및 그래프 옵션** (모든 diff 형식과 조합 가능):
- `--git-graph`: ASCII 및 Mermaid 커밋 그래프 시각화 포함 (git log --graph --all)
- `--git-summary`: 생성, 이름 변경, 모드 변경 등 파일 작업 표시 (git log --summary)
