...
| `output.splitOutput`             | Chia đầu ra thành nhiều tệp được đánh số theo kích thước tối đa mỗi phần. Khi sử dụng CLI, chỉ định kích thước dễ đọc như `500kb` hoặc `2mb`. Điều này giữ mỗi tệp dưới giới hạn và tránh chia các tệp nguồn giữa các phần | Không đặt |
