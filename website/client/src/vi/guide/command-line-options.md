...
- `--git-stat`: Hiển thị số lượng thay đổi theo từng file (git log --stat)
- `--git-patch`: Hiển thị diff theo từng dòng (git log --patch)
- `--git-numstat`: Hiển thị số thêm/xóa theo từng file (git log --numstat)
- `--git-shortstat`: Hiển thị tóm tắt một dòng về các thay đổi (git log --shortstat)
- `--git-dirstat`: Hiển thị phân bố thay đổi theo thư mục (git log --dirstat)
- `--git-name-only`: Chỉ hiển thị tên file (git log --name-only) - mặc định
- `--git-name-status`: Hiển thị tên file với trạng thái A/M/D/R (git log --name-status)
- `--git-raw`: Hiển thị định dạng cấp thấp với hash SHA và mode (git log --raw)

**Tùy chọn Mức độ Chi tiết Đầu ra và Đồ thị** (có thể kết hợp với bất kỳ định dạng diff nào):
- `--git-graph`: Bao gồm trực quan hóa đồ thị commit ASCII và Mermaid (git log --graph --all)
- `--git-summary`: Hiển thị các thao tác file như tạo, đổi tên, thay đổi mode (git log --summary)
